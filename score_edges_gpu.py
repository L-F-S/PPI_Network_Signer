...
linear algebra operations on pytorch vectors
"""
...
def propagate(PROPAGATE_ALPHA, PROPAGATE_ITERATIONS, PROPAGATE_EPSILON, seeds, matrix, gene_indexes, num_genes):
    ...
    for _ in range(PROPAGATE_ITERATIONS):
        ...
        F_t =  PROPAGATE_ALPHA*matrix.matmul(F_t_1) + Y 
        
        if math.sqrt(torch.norm(F_t_1 - F_t)) < PROPAGATE_EPSILON:
            break
    return F_t
...
def gpu_generate_features(raw_matrix, edges, gene_indexes, matrix, plus_targets_of_deletion, minus_targets_of_deletion, num_genes, PROPAGATE_ALPHA, PROPAGATE_ITERATIONS, PROPAGATE_EPSILON):
    ...
    '''
    shape=(len(edges),len(plus_targets_of_deletion.keys())*2)
    # tensorone = torch.zeros(shape).to(DEVICE)
    # tensorone = fill_tensor_with_function(tensorone, function)
