...
import subprocess
import sys
from glob_vars import SPECIES, PERT_MAP, HOME_DIR, LBL_DIR,\
    EDGES_DIR, FT_DIR, SIGNAL_DIR, MOD_DIR, ANAT_VAL_HOME, ANAT_VAL_DIR
...
INPUT_DIR =  ANAT_VAL_HOME+ '2 SIGNAL score networks'+os.sep
os.chdir(ANAT_VAL_HOME)
...
if SPECIES=='S_cerevisiae':
    ...
    f=open(ANAT_VAL_HOME+'1 ANAT Build networks'+os.sep+'Input'+os.sep+'TLManchors.txt')
    lines=[x.strip() for x in f.readlines()]
    ANCHORS=lines
    # ANCHORS=[str(alias_2geneid[node.upper()]) for node in ANCHORS]
    f.close()
    TLMphenotypesdf=pd.read_csv(ANAT_VAL_HOME+'1 ANAT Build networks'+os.sep+'Input'+os.sep+'TLMphenotypes.txt', header=0, sep='\t')
...
def generate_SIGNAL_features( edges_filename):
    ...
    command = ['python', HOME_DIR+'SIGNAL_ft_gen_iterative.py',
                '-e', edges_filename, '-s', SPECIES, '-p', PERT_MAP,
                '-fd', featurespath, '-ed', INPUT_DIR+'SIGNAL_input'+os.sep]
...
# Generate edges file (if already generated, set write=False)
