...
created to run once to make a n
"""
import os
import argparse
import pickle
...
with open(networkdir+SPECIES+'_'+datasetstr+'.nx', 'wb') as f:
    pickle.dump(graph,f)
    #%%test
with open(networkdir+SPECIES+'_'+datasetstr+'.nx', 'rb') as f:
    mygraph2=pickle.load(f)  
    #%%
print(len(graph.nodes)==len(mygraph2.nodes))
print(len(graph.edges)==len(mygraph2.edges))
