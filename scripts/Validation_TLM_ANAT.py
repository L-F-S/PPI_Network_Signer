...
ANATnet=nx.from_pandas_edgelist(SIGNAL_scores, source='id1',target='id2', edge_attr='SIGNAL')
# type_of={node: label(node) for node in ANATnet.nodes}    
# nx.set_node_attributes(ANATnet,type_of ,name='type')
#check
# nx.get_node_attributes(ANATnet, 'type')[855471]
...
for ANCHOR in ANCHORSid:
    if ANCHOR!= 850696 :#'TEN1': #TEN1, gene cher isultaa non connesso con gli altri nel net, quindi olo leiminiamo
        ...
        l_paths+=[nx.shortest_path(ANATnet, ANCHOR, node) for node in TERMS_in_subset if loose_label_of[node] == 1] 
        s_paths+=[nx.shortest_path(ANATnet, ANCHOR, node) for node in TERMS_in_subset if loose_label_of[node] == 0]
